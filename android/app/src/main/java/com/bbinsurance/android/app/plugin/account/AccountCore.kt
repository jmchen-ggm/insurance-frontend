 ...
 */
class AccountCore : NetListener {
    private val TAG = "BB.AccountCore"

    private var accountListenerList = ArrayList<IAccountListener>()

    fun addListener(listener: IAccountListener) {
        accountListenerList.add(listener)
    }

    fun removeListener(listener: IAccountListener) {
        accountListenerList.remove(listener)
    }

    override fun onNetDoneInMainThread(netRequest: NetRequest, netResponse: NetResponse) {
        for (listener : IAccountListener in accountListenerList) {
